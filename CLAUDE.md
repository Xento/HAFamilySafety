...
They are versioned and released together (integration `manifest.json` and add-on `config.json` should be at the same version — **they are currently out of sync: `manifest.json` is `1.1.47`, `config.json` is `1.1.2`**; fix this when cutting the next release). There is **no test suite** and **no linter config** in the repo; verification is manual against a real Microsoft Family Safety account.
...
- **Phase A — mobile OAuth.** The user signs in to Microsoft through a temporary reverse proxy mounted inside HA's own HTTP server (`auth/native_proxy.py`, `completion_mode="oauth"`). The proxy captures the `login.live.com/oauth20_desktop.srf?code=...` redirect → `refresh_token`, and keeps the Microsoft SSO cookies in its httpx cookie jar.
- **Phase B — Family web session.** The flow first tries a **server-side bootstrap** (`config_flow._try_server_side_family_bootstrap` → `FamilySafetyWebAPI._warm_family_context()`), reusing the phase-A SSO cookies to fetch `/family/windows/home/direct` and scrape `__RequestVerificationToken`. When that works the user never sees a second window. When it does not, the flow falls back to driving the *user's own browser* back through the proxy (`completion_mode="web"`; `_next_family_bootstrap_target()` walks three landing routes).

The browser fallback is **necessary and must stay**. A cold-start bootstrap with no SSO cookies cannot work, because Microsoft gates the Family SPA behind an interactive `prompt=none` OAuth hop (`login.microsoftonline.com/common/oauth2/v2.0/authorize`). At runtime the same condition surfaces as `api_client.py` `"Microsoft Family context requires browser authentication"` with `family_context_state = "auth_required"` and `last_web_error_code = "FAMILY_CONTEXT_AUTH_REQUIRED"`. Do not "simplify" phase B into a pure-httpx path.
...
There is **no autonomous renewal of the web session**. `sync_web_cookies_from_session()` captures Microsoft's `Set-Cookie` rotations on every 2xx/3xx, and `_warm_family_context()` re-scrapes the antiforgery token whenever it is empty, but an expired login can only be fixed by re-running the flow. `_request_reauth(reason)` calls `entry.async_start_reauth()` (HA de-duplicates) and a single persistent notification `familysafety_auth_expired` is raised.
...
- Upstream targets are restricted by `_is_allowed_host` to `live.com`, `microsoft.com`, `microsoftonline.com` (exact or dotted-suffix match). Never widen this — the view is unauthenticated, and widening turns it into an open proxy.
- `_extract_family_request_verification_token` accepts **only** `__RequestVerificationToken`. Do not fall back to `apiCanary`/`canary`: those belong to a different antiforgery context and yield 401. (`_web_canary` in `api_client.py` is vestigial — it is read but never assigned.)
...
- **`__init__.py`** registers the 17 services via a table-driven loop (`_register_services`): each row is `(name, schema, coordinator_method, args_extractor)`. To add a service: add the const + schema + a coordinator method + one table row. It also applies the two httpx patches before building the coordinator.
...
`user` → `start_mobile_auth` (external step opening `/auth/microsoft_family_safety/proxy/{token}`) → `check_mobile_proxy` → **server-side bootstrap**, or browser phase B reusing the *same* `check_mobile_proxy` step id (deliberate: the user must not press "Open website" twice) → `wait_family_proxy` (`async_show_progress`, `progress_action="family_sso"`) → `finish_proxy` → `auth_success` / `reauth_success`.
...
- The integration stays "degraded" (mobile API works, web session missing) rather than failing when web credentials are unavailable; `connection_state()` reports this and a persistent notification (`familysafety_auth_expired`) prompts re-auth. Only a real `LOGIN_REDIRECT` or an independently expired `/account` session triggers reauth — a bare 401 from the private Family API (`AUTH_ERROR`) while `/account` is still authenticated deliberately does **not**, to avoid a reauth loop caused by a merely stale antiforgery token.
...
- `entity_id`s are legacy-named: no entity sets `_attr_has_entity_name` or `_attr_translation_key`, and every `_attr_name` already embeds the child's name, so HA prefixes the slugified device name and the name appears twice (`number.maceo_collin_family_safety_maceo_sunday_limit`). Also note the device name differs by platform: `sensor.py`/`number.py`/`time.py` use `"{first} {surname} (Family Safety)"` while `switch.py`/`button.py` use `"{first} (Family Safety)"`. Changing any of this renames entities for every user — treat it as a breaking change.
